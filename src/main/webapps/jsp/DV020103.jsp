...
function fn_InitGrid() {
	var caption = "TASK ID^TASK 명";
	var colWidth = "40%,60%";
	
	objGridInitInfoStat = fn_InitSBGrid("prjtTaskInfo", "objSBGridStat", caption, colWidth, "250px", "resultDataStat", true, null);
	...
	objSBGridStat.addEventListener("onclick", "fn_SelectTaskInfo", "","");

	caption = "SEG ID^SEG 명^시작일자^종료일자^";
	colWidth = "20%,40%,20%,20%,20%";
	
	objGridInitInfo = fn_InitSBGrid("prjtSegInfo", "objSBGrid", caption, colWidth, "250px", "resultData", true, null);
	objSBGrid = createSBDataGrid(objGridInitInfo);

	objColumnsInfo = [
	  				fn_SetSBGridCol("00", "output", "sSEG_ID" , "center", null, false),
	  				fn_SetSBGridCol("01", "output", "sSEG_NM" , "left"  , null, false),
	...
	caption = "*TASK ID^*SEG ID^*SEG SUB ID^SEG SUB 명^업무내용^담당자^담당자^시작일자^종료일자^범위확정^";
	colWidth = "12%,12%,15%,23%,19%,12%,3%,13%,13%,10%";
	
	objGridInitInfoDetail = fn_InitSBGrid("prjtSegSubInfo", "objSBGridDetail", caption, colWidth, "400px", "resultDataDetail", true, null);
	objSBGridDetail = createSBDataGrid(objGridInitInfoDetail);

	objColumnsInfoDetail = [
		  				fn_SetSBGridCol("00", "output"    , "sTASK_ID"      , "center", null, false),
		  				fn_SetSBGridCol("01", "output"    , "sSEG_ID"       , "center", null, false),
		  				fn_SetSBGridCol("02", "input"     , "sSEG_SUB_ID"   , "center", null, false),
		  				fn_SetSBGridCol("03", "input"     , "sSEG_SUB_NM"   , "left"  , null, false),
		  				fn_SetSBGridCol("04", "input"     , "sSEG_SUB_DESC" , "left"  , null, true),
		  				fn_SetSBGridCol("05", "output"    , "sCORP_MNG_NM"  , "left"  , null, false),
...
function fn_Init_Clear() {
	window.location = "DV020103.jsp";
};

function fn_Init_Data() {
	$("#section input[type=text]:not([id=sTASK_ID]), #section select, #section textarea").val("");
};
...
function fn_SelectInitID() {
	...
	fn_AjaxSend(sendData, "DV020102B", fn_SetDataSelectSegID, null);
};

function fn_SetDataSelectSegID(result) {
	var segSubInfo = result.segSubInfo;
	$("#sSEG_SUB_ID").val(segSubInfo.sSEG_SUB_ID);
};
...
function fn_SetDataSelectPrjtInfo(result) {
	resultDataStat = result.taskInfo;
...
function fn_InsertPrjt() {
	...
	var sendData = {
		...
		sSEG_SUB_NM : $("#sSEG_SUB_NM").val(),
		sSEG_SUB_DESC : $("#sSEG_SUB_DESC").val(),
	...
	fn_AjaxSend(sendData, "DV020121B", fn_SelectListInfo, PMS_PROC_INSERT);
};
...
function fn_DeletePrjt() {
	...
	var sendData = {
		...
		sPRJT_STAGE : $("#searchPrjtStage option:selected").val(),
		sTASK_ID : $("#sTASK_ID").val(),
		sSEG_ID : objSBGrid.getTextMatrix(nClickedRow,1),
		sSEG_SUB_ID : $("#sSEG_SUB_ID").val()
	};

	fn_AjaxSend(sendData, "DV020141", fn_SelectListInfo, PMS_PROC_DELETE);
};
...
function fn_ModifyPrjt() {
	...
	arrMap.put("sSEG_ID"	, $("#sSEG_ID").val());
	
	for (var i=1; i<objSBGridDetail.rows; i++) {
		
		var taskMap = new Map();
		
		taskMap.put("sSEG_SUB_ID"	, objSBGridDetail.getTextMatrix(i,2));
		taskMap.put("sSEG_SUB_NM"	, objSBGridDetail.getTextMatrix(i,3));
		taskMap.put("sSEG_SUB_DESC"	, objSBGridDetail.getTextMatrix(i,4));
		taskMap.put("sSTRT_DT"      , objSBGridDetail.getTextMatrix(i,7));
		taskMap.put("sEND_DT"       , objSBGridDetail.getTextMatrix(i,8));
		taskMap.put("sDEF_YN"      	, objSBGridDetail.getTextMatrix(i,9));
		taskMap.put("sCORP_MNG_ID"	, objSBGridDetail.getTextMatrix(i,10));
		
		arrlist[i-1] = taskMap;
	}
	
	arrMap.putMapList("sPRJT_TASK_SEG_SUB_INFO", arrlist);
	
	var sendData = arrMap.jsonParse();
	fn_AjaxSend(sendData, "DV020122B", fn_SelectListInfo, PMS_PROC_INSERT);
};

function fn_SelectTaskInfo() {
	
	var nClickedRow = objSBGridStat.getRow();
	if (nClickedRow <= 0 )
		return;
	
	$("#sTASK_ID").val(objSBGridStat.getTextMatrix(nClickedRow,0));
	$("#sSEG_ID").val("");
	$("#sSEG_SUB_ID").val("");
	...
	fn_AjaxSend(sendData, "DV020111A", fn_SetDataSelectTaskInfo, null);
};

function fn_SetDataSelectTaskInfo(result) {
	resultData = result.segInfo;
	objSBGrid.rebuild();
};
...
function fn_SelectListInfo() {
	...
	$("#sSEG_ID").val(objSBGrid.getTextMatrix(nClickedRow,0));
	...
	fn_AjaxSend(sendData, "DV020111B", fn_SetDataSelectListInfo, null);
};

function fn_SetDataSelectListInfo(result) {
	resultDataDetail = result.segSubInfo;
...
function fn_SelectListInfoDetail() {
	...
	$("#sSEG_SUB_ID").val(objSBGridDetail.getTextMatrix(nClickedRow,2));
	$("#sSEG_SUB_NM").val(objSBGridDetail.getTextMatrix(nClickedRow,3));
	$("#sSEG_SUB_DESC").val(objSBGridDetail.getTextMatrix(nClickedRow,4));
	$("#sCORP_MNG_NM").val(objSBGridDetail.getTextMatrix(nClickedRow,5));
	$("#sSTRT_DT").val(fn_SetDateType(objSBGridDetail.getTextMatrix(nClickedRow,7)));
	$("#sEND_DT").val(fn_SetDateType(objSBGridDetail.getTextMatrix(nClickedRow,8)));
	$("#sDEF_YN").val(objSBGridDetail.getTextMatrix(nClickedRow,9));
	$("#sCORP_MNG_ID").val(objSBGridDetail.getTextMatrix(nClickedRow,10));
	
	var nSelCol = objSBGridDetail.col;
	var nSelRow = objSBGridDetail.row;
	
	var popupUrl = "PU010112.jsp?nSelRow="+nSelRow+"&DV020103=sGRID_INFO";
...
function fn_AddRow() {
	...
	if(objSBGridDetail.selectedRow(0) == '-1'){
		...
		objSBGridDetail.setTextMatrix(objSBGridDetail.getRows()-1,1,$("#sSEG_ID").val());
	}else{
		...
		objSBGridDetail.setTextMatrix(objSBGridDetail.selectedRow(0),1,$("#sSEG_ID").val());
	}
};
...
function fn_SearchUser() {
	var popupUrl = "PU010112.jsp?DV020103=sTEXT_INFO";
	window.open(popupUrl,"",popupStyle);
};
