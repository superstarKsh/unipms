...
function fn_SelectPrjtInfo() {
	...
	fn_AjaxSend(sendData, "MG140111", fn_SetDataSelectPrjtInfo, null);
};

function fn_SetDataSelectPrjtInfo(result) {
	$.each(result.mngInfo,function(key, value) {
		var temp = document.getElementById(value.sACTIVITY_ID);

		if(temp != null){
			temp.setAttribute("style", "background-color:#d7908c;");
		}
	});
};

function fn_SearchPage(args) {
	
	var content = "";
	content += "sSEARCH_YN=Y";
	content += "&";
	content += "sSEARCH_ACTIVITY_ID="+ args;

	fn_AjaxSendSesUrl(content, "pms_session_mg1401.jsp", "MG130101.jsp", null);
};
