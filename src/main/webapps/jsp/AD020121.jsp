...
function fn_InitGrid() {
	...
	var caption = "순번^학교명^학과^졸업년월";
	var colWidth = "10%,30%,40%,20%,0%";
	...
	objColumnsInfo = [
		...
		fn_SetSBGridCol("01", "input", "sSCHL_NM"       , "center", null, false),
		fn_SetSBGridCol("02", "input", "sSCHL_DEPT"     , "center", null, false),
		fn_SetSBGridCol("03", "input", "sGRAD_YYYYMM"   , "center", "yyyy.mm", false)
	];
...
function fn_SelectPrjtInfo() {
	...
	fn_AjaxSend(sendData, "AD020111A", fn_SetDataSelectPrjtInfo, null);
};

function fn_SetDataSelectPrjtInfo(result) {
	resultData = result.academicInfo;
    objSBGrid.rebuild();
};
...
function fn_ModifyPrjt() {
	...
	for (var i=1; i < objSBGrid.rows; i++) {
		...
		taskMap.put("sSCHL_NM"       , objSBGrid.getTextMatrix(i,1));
		taskMap.put("sSCHL_DEPT"     , objSBGrid.getTextMatrix(i,2));
		taskMap.put("sGRAD_YYYYMM"   , objSBGrid.getTextMatrix(i,3));
		
		arrlist[i-1] = taskMap;
	}

	arrMap.putMapList("sACADEMIC_INFO", arrlist);

	var sendData = arrMap.jsonParse();
	fn_AjaxSend(sendData, "AD020122A", fn_SelectPrjtInfo, PMS_PROC_INSERT);
};
