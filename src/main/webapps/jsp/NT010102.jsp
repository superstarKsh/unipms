...
function fn_SessionCheck() {
	...
	if ($("#sSEARCH_YN").val() == 'Y') {
		...
		fn_AjaxSendSesUrl(content, "pms_session_nt0101.jsp", null, fn_SelectPrjtInfo);
	}
};
...
function fn_SelectPrjtInfo() {
	...
	fn_AjaxSend(sendData, "NT010113", fn_SetDataSelectPrjtInfo, null);
};

function fn_SetDataSelectPrjtInfo(result) {
	if (result.noticeInfo != null && result.noticeInfo != 'undefined') {
		var noticeInfo = result.noticeInfo;
				
		$("#sREG_DT").val(fn_SetDateType(noticeInfo.sREG_DT));
		$("#sREG_USER_ID").val(noticeInfo.sREG_USER_ID);
		$("#sREG_USER_NM").val(noticeInfo.sREG_USER_NM);
		$("#sTITLE").val(noticeInfo.sTITLE);
		$("#sSEQ_NO").val(noticeInfo.sSEQ_NO);
		$("#sCONTENTS").val(noticeInfo.sCONTENTS);
		$("#sFILE_NM").val(noticeInfo.sFILE_NM);
		$("#FILE_NM").val(noticeInfo.sFILE_NM);
		$("#sPRINT_TOP_YN").attr("checked", noticeInfo.sPRINT_TOP_YN=="Y"?true:false);
		
		if (noticeInfo.sPRINT_ST_DTTM != '') {
			$("#sPRINT_STDT").val(fn_SetDateType(noticeInfo.sPRINT_ST_DTTM.substr(0, 8)));
			$("#sPRINT_STTM").val(fn_SetDateType(noticeInfo.sPRINT_ST_DTTM.substr(8, 2) + ":" + noticeInfo.sPRINT_ST_DTTM.substr(10, 2)));	
		}
		
		if (noticeInfo.sPRINT_EN_DTTM != '') {
			$("#sPRINT_ENDT").val(fn_SetDateType(noticeInfo.sPRINT_EN_DTTM.substr(0, 8)));
			$("#sPRINT_ENTM").val(fn_SetDateType(noticeInfo.sPRINT_EN_DTTM.substr(8, 2) + ":" + noticeInfo.sPRINT_EN_DTTM.substr(10, 2)));
		}
		
		if ($("#sREG_USER_ID").val() != $("#sUSER_ID").val()) {			
			$("#trPrintContent1, #trPrintContent2").remove();
			var tdContents = document.getElementById("tdContents");
			var contentsStr = "";
			contentsStr += "<ul class='cont'>";
			contentsStr += noticeInfo.sCONTENTS;
			contentsStr += "</ul>";
			tdContents.innerHTML = contentsStr;
			
			var inputFileStr = "";
			inputFileStr += "<span style='cursor:pointer' onclick='javascript:fn_FileDownload()'>";
			inputFileStr += noticeInfo.sFILE_NM;
			inputFileStr += "</span>";
			$("#tdInputFile").html(inputFileStr);
			
			$("#btnInit, #btnSave, #btnDelete").css("display","none");
		} 
	}
...
function fn_InsertPrjt() {
	...
	var sendData = {
		...
		sPRINT_TOP_YN : $("#sPRINT_TOP_YN").is(":checked")==true?"Y":"N",
		sPRINT_ST_DTTM : $("#sPRINT_STDT").val() + $("#sPRINT_STTM").val(),
		sPRINT_EN_DTTM : $("#sPRINT_ENDT").val() + $("#sPRINT_ENTM").val()
	};

	fn_AjaxSend(sendData, "NT010121", fn_MoveToList, PMS_PROC_INSERT);
};
...
function fn_DeletePrjt() {
	...
	fn_AjaxSend(sendData, "NT010141", fn_MoveToList, PMS_PROC_DELETE);
};

function fn_InsertPrjtFile() {
	fn_InsertUploadFile("NT01", fn_InsertPrjt);
};

function fn_FileDownload() {
	var downloadFile = $("#FILE_NM").val();
	if (downloadFile == "")
		return;
...
$(function() {
    $("#sPRINT_STDT").datepicker(setCalendar);
});

$(function() {
    $("#sPRINT_ENDT").datepicker(setCalendar);
});
