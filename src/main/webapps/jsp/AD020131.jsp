...
function fn_InitGrid() {
	...
	var caption = "순번^자격증명^취득년월^발행기관";
	var colWidth = "10%,40%,20%,30%";
	...
	objColumnsInfo = [
		...
		fn_SetSBGridCol("01", "input", "sCERTIFCT_NM"   , "left"  , null, false),
		fn_SetSBGridCol("02", "input", "sACQ_YYYYMM"    , "center", "yyyy.mm", false),
		fn_SetSBGridCol("03", "input", "sPUB_ORG_NM"    , "left"  , null, false)
	];
...
function fn_SelectPrjtInfo() {
	...
	fn_AjaxSend(sendData, "AD020111B", fn_SetDataSelectPrjtInfo, null);
};

function fn_SetDataSelectPrjtInfo(result) {
	resultData = result.certifctInfo;
	objSBGrid.rebuild();
};
...
function fn_ModifyPrjt() {
	...
	for (var i=1; i < objSBGrid.rows; i++) {
		...
		taskMap.put("sCERTIFCT_NM"  , objSBGrid.getTextMatrix(i,1));
		taskMap.put("sACQ_YYYYMM"   , objSBGrid.getTextMatrix(i,2));
		taskMap.put("sPUB_ORG_NM"   , objSBGrid.getTextMatrix(i,3));
		
		arrlist[i-1] = taskMap;
	}
	
	arrMap.putMapList("sCERTIFCT_INFO", arrlist);

	var sendData = arrMap.jsonParse();
	fn_AjaxSend(sendData, "AD020122B", fn_SelectPrjtInfo, PMS_PROC_INSERT);
};
