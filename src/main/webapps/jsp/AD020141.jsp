...
function fn_InitGrid() {
	...
	var caption = "순번^교육과정명^시작일자^종료일자^교육기관";
	var colWidth = "10%,30%,15%,15%,30%";
	...
	objColumnsInfo = [
		...
		fn_SetSBGridCol("01", "input", "sEDUT_NM"       , "left"  , null, false),
		fn_SetSBGridCol("02", "input", "sEDUT_STRT_DT"  , "center", "yyyy.mm.dd", false),
		fn_SetSBGridCol("03", "input", "sEDUT_END_DT"   , "center", "yyyy.mm.dd", false),
		fn_SetSBGridCol("04", "input", "sEDUT_ORG_NM"   , "left"  , null, false)
	];
...
function fn_SelectPrjtInfo() {
	...
	fn_AjaxSend(sendData, "AD020111C", fn_SetDataSelectPrjtInfo, null);
};

function fn_SetDataSelectPrjtInfo(result) {
	resultData = result.edutInfo;
	objSBGrid.rebuild();
};
...
function fn_ModifyPrjt() {
	...
	for (var i=1; i < objSBGrid.rows; i++) {
		...
		taskMap.put("sEDUT_NM"      , objSBGrid.getTextMatrix(i,1));
		taskMap.put("sEDUT_STRT_DT" , objSBGrid.getTextMatrix(i,2));
		taskMap.put("sEDUT_END_DT"  , objSBGrid.getTextMatrix(i,3));
		taskMap.put("sEDUT_ORG_NM"  , objSBGrid.getTextMatrix(i,4));
		
		arrlist[i-1] = taskMap;
	}

	arrMap.putMapList("sEDUT_INFO", arrlist);

	var sendData = arrMap.jsonParse();
	fn_AjaxSend(sendData, "AD020122C", fn_SelectPrjtInfo, PMS_PROC_INSERT);
};
